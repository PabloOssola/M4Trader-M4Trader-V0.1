...
SELECT 
	  CAST(CONVERT(VARCHAR,cast((round(hc.Fecha/100,0,1)*100) + 1 as INT),112) AS DATETIME) Fecha
  ...
  AND hc.rueda != @RuedaExcludia AND Estado = @EstadoOperaciones
  and hc.moneda in (@Moneda1,@Moneda2) and not tipo in ('@', '+','=')
  GROUP BY CAST(CONVERT(VARCHAR,cast((round(hc.Fecha/100,0,1)*100) + 1 as INT),112) AS DATETIME), hc.codigodeti, p.IdProducto
  ORDER BY CAST(CONVERT(VARCHAR,cast((round(hc.Fecha/100,0,1)*100) + 1 as INT),112) AS DATETIME), hc.codigodeti, p.IdProducto
...
BEGIN
	SELECT @FechaMinima = MIN(FECHA), @FechaMaxima = MAX(FECHA) FROM [LINK_SERVER_SIOPEL].Siopel_1001.siopel_owner.swoper_historico hc
	WHERE FechaAlterada >= @Fecha AND FechaAlterada < dateadd(mm,1, @Fecha) 
		AND hc.rueda != @RuedaExcludia AND Estado = @EstadoOperaciones
...
SELECT 
Fecha, DATEADD(ms, -2, DATEADD(mm, 1, Fecha)), IdProducto, IdMoneda,
