...
namespace M4Trader.ordenes.server.DataAccess.Interfaces
{
    public static class MappeableReader
    {
        ...
        public static List<T> DataReadersMapToEntity<T>(this IDataReader dr) where T : new()
        {
                ...
                foreach (PropertyInfo prop in obj.GetType().GetProperties())
                {
                    if (columns.IndexOf(prop.Name.ToLower()) >= 0)
                    {
                        if (!object.Equals(dr[prop.Name], DBNull.Value))
                        {
                            ...
                            {
                                if (dr.GetFieldType(dr.GetOrdinal(prop.Name)) == typeof(Byte[]))
                                {
                                    ...
                                    {
                                        value = System.BitConverter.ToString((byte[])dr[prop.Name], 0);
                                    }
                                }
                                else
                                {
